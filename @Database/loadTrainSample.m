function [imgs, gt, valid, raw_imgs] = loadTrainSample(obj, fidx, varargin)
    ...
    raw_imgs = cell(1,4);
    for i = 1:4
        I = imread(obj.imgs_path{i}{obj.train_idx(fidx)});
        raw_imgs{i} = I;
        if size(I, 3) == 3
            I = rgb2gray(I);
        end
    ...
    if nargout >= 2 && strcmp(obj.dtype, 'gt') && ...
            length(obj.train_depth_path) >= fidx
        gtsize = obj.train_depth_size(fidx,:);
        gt = single(imread((obj.train_depth_path{fidx})));
